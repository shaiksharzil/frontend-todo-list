      ...
      <div className="flex justify-center items-center w-screen h-screen fixed">
        <div className="fixed border-2 bg-white border-[#EEEEEE] w-90 rounded-md mx-4 flex flex-col">
          <div className="mx-3 my-4 px-2 rounded-sm">
                    <h3 className='text-xl'>Are you sure you want to delete <strong>{t.task}</strong>?</h3>
          </div>
          <div className="flex justify-end mt-5 gap-4 mr-2 mb-4">
            <button onClick={()=>setDelPopUp(false)} className="bg-emerald-400 text-white rounded-md px-3 py-0.5 text-lg">
              No
            </button>
            <button onClick={()=>handleDelete(t._id)} className="bg-red-400 rounded-md text-white px-3 py-0.5 text-lg">
              Yes
            </button>
          </div>
        </div>
