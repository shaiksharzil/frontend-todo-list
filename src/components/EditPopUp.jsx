...
const EditPopUp = ({ t, setEditPopUp, onConfirm, onCancel }) => {
  ...
  return (
    <div className="flex justify-center items-center w-screen h-screen fixed top-0 left-0 z-50">
      <div className="border-2 bg-white border-[#EEEEEE] w-90 rounded-md mx-4 flex flex-col">
        ...
        <input
          ...
          className="border-2 border-[#EEEEEE] mx-3 rounded-md pl-2 outline-none"
        />
        <div className="flex justify-end mt-5 gap-4 mr-2 mb-4">
          <button
            onClick={onCancel}
            className="bg-emerald-400 text-white rounded-md px-3 py-0.5 text-lg"
          >
            No
          </button>
          <button
            onClick={() => onConfirm(newTask)}
            className="bg-red-400 rounded-md text-white px-3 py-0.5 text-lg"
          >
            Yes
          </button>
        </div>
